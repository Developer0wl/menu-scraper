...
/**
 * Firehouse Subs Allergen Scraper
 *
 * URL: https://www.firehousesubs.com/nutrition
 *
 * Strategy:
 *   1. Navigate to /nutrition page
 *   2. Look for allergen table or per-item allergen disclosure
 *   3. Each sub listing typically shows "Contains: X, Y, Z" inline
 *   4. Extract all items and map allergens
 *   5. Fall back to known items with CNV if the page is inaccessible
 */
...
const OFFICIAL_URL = 'https://www.firehousesubs.com/nutrition';
const ALT_URL      = 'https://www.firehousesubs.com/menu/';
...
const KNOWN_ITEMS = [
  { name: 'Hook & Ladder',              category: 'Hot Subs' },
  { name: 'Firehouse Hero',             category: 'Hot Subs' },
  { name: 'Engineer',                   category: 'Hot Subs' },
  { name: 'New York Steamer',           category: 'Hot Subs' },
  { name: 'Italian',                    category: 'Hot Subs' },
  { name: 'Meatball',                   category: 'Hot Subs' },
  { name: 'Chicken Bacon Ranch',        category: 'Hot Subs' },
  { name: 'BBQ Brisket',                category: 'Hot Subs' },
  { name: 'Smokehouse Beef & Cheddar Brisket', category: 'Hot Subs' },
  { name: 'Primo Italiano',             category: 'Hot Subs' },
  { name: 'Turkey Bacon Ranch',         category: 'Cold Subs' },
  { name: 'Club on a Sub',              category: 'Cold Subs' },
  { name: 'Veggie',                     category: 'Cold Subs' },
  { name: 'Tuna',                       category: 'Cold Subs' },
  { name: 'Firehouse Salad',            category: 'Salads' },
  { name: 'Chicken Salad',              category: 'Salads' },
];

class FirehouseSubs extends BaseScraper {
  constructor() {
    super({ chainName: 'FirehouseSubs', officialUrl: OFFICIAL_URL });
    this._headers = null;
  }
  ...
  async extractAllergens(item) {
    ...
    row.sourceText   = 'Allergen data not accessible — check firehousesubs.com/nutrition';
    return row;
  }
}

module.exports = FirehouseSubs;
